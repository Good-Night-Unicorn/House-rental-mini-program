<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="设备配置" prop="shebeipeizhi">
          <el-input v-model="ruleForm.shebeipeizhi" 
              placeholder="设备配置" clearable  :readonly="ro.shebeipeizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="设备配置" prop="shebeipeizhi">
              <el-input v-model="ruleForm.shebeipeizhi" 
                placeholder="设备配置" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="装修情况" prop="zhuangxiuqingkuang">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.zhuangxiuqingkuang" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhuangxiuqingkuang" label="装修情况" prop="zhuangxiuqingkuang">
                    <span v-html="ruleForm.zhuangxiuqingkuang"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.zhuangxiuqingkuang = this.ruleForm.zhuangxiuqingkuang.replace(reg,'../../../ssmg90dl/upload');
	this.ruleForm.fangwuxiangqing = this.ruleForm.fangwuxiangqing.replace(reg,'../../../ssmg90dl/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fangyuanguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangyuanguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
