<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('fangwuxinxi','信息录入') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="fangyuanguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'信息录入':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','信息录入') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="fangyuanguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'信息录入':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','信息录入') && contents.tableBtnIcon == 0" type="success" size="mini" @click="fangyuanguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'信息录入':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','租房') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="zufangdingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租房':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','租房') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="zufangdingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租房':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','租房') && contents.tableBtnIcon == 0" type="success" size="mini" @click="zufangdingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租房':'' }}</el-button>
    ...
    <fangyuanguanli-cross-add-or-update v-if="fangyuanguanliCrossAddOrUpdateFlag" :parent="this" ref="fangyuanguanliCrossaddOrUpdate"></fangyuanguanli-cross-add-or-update>
    <zufangdingdan-cross-add-or-update v-if="zufangdingdanCrossAddOrUpdateFlag" :parent="this" ref="zufangdingdanCrossaddOrUpdate"></zufangdingdan-cross-add-or-update>
...
import fangyuanguanliCrossAddOrUpdate from "../fangyuanguanli/add-or-update";
import zufangdingdanCrossAddOrUpdate from "../zufangdingdan/add-or-update";
export default {
  data() {
    return {
      ...
      fangyuanguanliCrossAddOrUpdateFlag: false,
      zufangdingdanCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    fangyuanguanliCrossAddOrUpdate,
    zufangdingdanCrossAddOrUpdate,
  },
  methods: {
    contentStyleChange() {
    ...
    fangyuanguanliCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.fangyuanguanliCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','fangwuxinxi');
      this.$nextTick(() => {
      this.$refs.fangyuanguanliCrossaddOrUpdate.init(row.id,type);
      });
    },
    zufangdingdanCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.zufangdingdanCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','fangwuxinxi');
      this.$nextTick(() => {
      this.$refs.zufangdingdanCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
