...
export default {
  data() {
	...
	nianling : false,
      ...
      rules: {
          zhongjiezhanghao: [
                { required: true, message: '中介账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          zhongjiexingming: [
                { required: true, message: '中介姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          touxiang: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhongjie/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhongjieCrossAddOrUpdateFlag = false;
                  this.parent.search();
